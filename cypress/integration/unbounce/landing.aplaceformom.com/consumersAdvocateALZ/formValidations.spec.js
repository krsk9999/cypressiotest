	...
	context("Form Validations and Lead submissions", () => {
		...
		before(() => {
			cy.fixture("UnbounceData.json").then(d => {
                ...
                variant = domainsData.landingConsumersAdvocateALZ.variants.a.url
                touUrl = domainsData.landingConsumersAdvocateALZ.variants.a.touUrl
                ppUrl = domainsData.landingConsumersAdvocateALZ.variants.a.ppUrl
			})
        ...
        it("Validate Therms of Use link", () => {
            ...
            cy.get(`a[href="clkn/https/www.alzheimers.net/terms-of-use/"]`).click()
        ...
        it("Validate Privacy Policy link", () => {
			...
			let variant = domainsData.landingConsumersAdvocateALZ.variants.a.url
            
			cy.navigate(url, variant)
            
            cy.get(`a[href="clkn/https/www.alzheimers.net/privacy-policy/"]`).click()
            
            cy.url().should("eq",ppUrl)
		})

        it("Fields are required variant 'a'", () => {	
            let typUrl = domainsData.landingConsumersAdvocateALZ.variants.a.tyPage
			let pagePhone = domainsData.landingConsumersAdvocateALZ.variants.a.phone
			let fullUrl = url + variant
			let bodyTag = domainsData.landingConsumersAdvocateALZ.variants.a.bodyTag
            let responseBody
            let sourceId = domainsData.landingConsumersAdvocateALZ.variants.a.sourceId
            ...
            //Phone number
            cy.get('#lp-pom-text-13 p span').as("phone").should("exist")
            cy.get("@phone").invoke("text").then((text)=>{
            ...
            mainPage.getLocationElement.as("location").should("not.have.class", "error")
            mainPage.getNameElement.as("name").should("not.have.class", "error")
            mainPage.getEmailElement.as("email").should("not.have.class", "error")
            mainPage.getPhoneElement.as("phone").should("not.have.class", "error")
            ...
            //Fill the Lead form
            mainPage.getLocationElement.type(location).should("have.value",location)
            ...
            cy.url().should("eq","https://www.alzheimers.net/find-dementia-care/")
        })
