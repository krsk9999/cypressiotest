import iDot from "../../../models/pages/aplaceformom/iDot/iDotP.js";
import iDotTY from "../../../models/pages/aplaceformom/iDot/iDotTYP.js";

describe("IDot form validations", () => {
	context("Form Validations and Lead submissions variant 'P'", () => {
		let domainsData;
		let url;
		let variant;
		let bodyTag;
		...
		before(() => {
			cy.fixture("UnbounceData.json").then(d => {
				domainsData = d.iDot;
				url = domainsData.url;
				variant = domainsData.variants.p.url;
				bodyTag = domainsData.variants.p.bodyTag;
				sourceId = domainsData.variants.p.sourceId;
				creativeId = domainsData.variants.p.creativeId;
				dataPaFormId = domainsData.variants.p.dataPaFormid;
				dataPaFormType = domainsData.variants.p.dataPaFormtype;
				dataPaTechStack = domainsData.variants.p.dataPaTechstack;
				aboutUs = domainsData.variants.p.aboutUs;
				ppUrl = domainsData.variants.p.ppUrl;
				touUrl = domainsData.variants.p.touUrl;
			});
		...
		it("Fields are required", () => {
			...
			mainPage.getLocationErrorElement.as("locationError");
			mainPage.getCareTypeElement.as("careType");
			cy.get("@careType").should("have.text", "Senior Living");
			cy.get("@location").should("not.have.class", "error");
			cy.get("@nextBtn").click();

			cy.url().should("eq", url + variant);
			cy.get("@location").should("have.class", "error");
			cy.get("@locationError").should(
				"have.text",
			...
			cy.get("@selections").get("select#timeline").as("timeLine");
			cy.get("@timeLine").select("within 60 days");
			cy.get("@selections").get("select#room_type").as("roomType");
			cy.get("@roomType").select("private one bedroom");
			cy.get("@selections").get("select#budget").as("budget");
